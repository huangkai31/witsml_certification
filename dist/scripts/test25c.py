...
test(
     purpose = "Verify that the server truncates data such that no data is returned outside of the truncation interval using maxDataPoints for a time log.",
     ...
     functionality_required =   ["WMLS_GetFromStore:log",
                                 "WMLS_GetFromStore:log:maxDataPoints<100000000"],
...
WMLS_GetFromStore(WMLTYPEIN_LOG, """<?xml version="1.0" encoding="UTF-8"?>
                                      <logs xmlns="http://www.witsml.org/schemas/1series" version="$server_schema_version$">
                                        <log uidWell="$server_w3_uid$" uidWellbore="$server_w3_wb1_uid$" uid="$server_w3_wb1_log4_uid$">                       
                                        </log>
                                      </logs>
                                  """,
